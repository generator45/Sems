...
int pds_load_ndx()
{
    ...
    for (i = 0; i < repo_handle.rec_count; i++)
    {
        ...
        bst_add_node(&repo_handle.ndx_root, ndx->key, ndx);
    }
...
int put_rec_by_key(int key, void *rec)
{
    ...
    if (bst_add_node(&repo_handle.ndx_root, key, ndx) != BST_SUCCESS)
    {
...
int get_rec_by_field(void *searchvalue, void *rec, int (*matcher)(void *rec, void *searchvalue), int *comp_count)
{
    ...
    *comp_count = 0;
    ...
    while (fread(&filekey, sizeof(int), 1, fp) > 0)
    {
        ...
        {
            // not deleted mark
            struct BST_Node *result = bst_search(repo_handle.ndx_root, filekey);
            if (result == NULL)
            {
                return PDS_REC_NOT_FOUND;
            }
...
// pds_make_index
// given data file, create an index file
// int make_index(FILE *fp)
// {
