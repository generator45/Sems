...
int pds_create_db(char* dbname) {
    ...
    char filename[30];
    sprintf(filename, "%s/%s.info", dbname, dbname);
    FILE* fp1;
    ...
    char db[30] = "";
    strcat(db, dbname);
    if (fwrite(db, sizeof(char), sizeof(repo_handle.pds_repo_name), fp1) < sizeof(repo_handle.pds_repo_name)) {
        return PDS_FILE_ERROR;
    }
...
int pds_open_db(char* dbname) {
    ...
    char name[30];
    fread(name, sizeof(char), sizeof(repo_handle.pds_repo_name), fp);
    printf("%s, \n", name);
    ...
    char table_name[30];
    int rec_size;
    while (fread(table_name, sizeof(char), 30, fp) > 0) {
...
int pds_create_table(char* table_name, int rec_size) {
    ...
    char datafile[30] = "";
    char indexfile[30] = "";
    strcpy(datafile, table_name);
    strcat(datafile, ".dat");
    strcpy(indexfile, table_name);
    strcat(indexfile, ".ndx");
    FILE* dfp = fopen(datafile, "wb");
    FILE* ifp = fopen(indexfile, "wb");
    char name[30];
    strcpy(name, repo_handle.pds_repo_name);
    strcat(name, ".info");
    // sprintf(name,"%s.info",repo_handle.pds_repo_name);
    int zero = 0;;
    fwrite(&zero, sizeof(int), 1, ifp);
    fclose(dfp);
    fclose(ifp);
    ...
    fwrite(table, sizeof(char), 30, fp);
...
int pds_open_table(char* table_name, int rec_size) {
    ...
    strcpy(indexfile, table_name);
    strcat(indexfile, ".ndx");
    FILE* dfp = fopen(datafile, "rb+");
    FILE* ifp = fopen(indexfile, "rb+");
    ...
    if (table != NULL) {
        ...
        strcpy(repo_handle.tables[repo_handle.num_tables].table_name, table_name);
        repo_handle.tables[repo_handle.num_tables].data_fp = dfp;
        repo_handle.tables[repo_handle.num_tables].ndx_fp = ifp;
        repo_handle.tables[repo_handle.num_tables].rec_size = rec_size;
        repo_handle.tables[repo_handle.num_tables].table_status = PDS_TABLE_OPEN;
        repo_handle.tables[repo_handle.num_tables].rec_count = 0;
        repo_handle.num_tables++;
    }
    struct PDS_TableInfo* new = helper_get_table_info(table_name);
    pds_load_ndx(new);
    fclose(ifp);
    return PDS_SUCCESS;
}
...
int pds_load_ndx(struct PDS_TableInfo* tablePtr) {
    int count = 0;
    fread(&count, sizeof(int), 1, tablePtr->ndx_fp);
    tablePtr->ndx_root = NULL;
    for (int i = 0;i < count;i++) {
        struct PDS_NdxInfo* entry = (struct PDS_NdxInfo*)malloc(sizeof(struct PDS_NdxInfo));
        fread(entry, sizeof(struct PDS_NdxInfo), 1, tablePtr->ndx_fp);
        bst_add_node(&tablePtr->ndx_root, entry->key, entry);
    }
    return PDS_SUCCESS;
}
//////////////////////////////////////////////////////////////////////////////
...
int put_rec_by_key(char* table_name, int key, void* rec) {
    ...
    int offset = ftell(table->data_fp);
    struct PDS_NdxInfo* entry = (struct PDS_NdxInfo*)malloc(sizeof(struct PDS_NdxInfo));
    entry->key = key;
    entry->offset = offset;
    entry->is_deleted = REC_NOT_DELETED;
    if (bst_search(table->ndx_root, key) != NULL) {
        return PDS_ADD_FAILED;
    }
    bst_add_node(&table->ndx_root, entry->key, entry);
    table->rec_count++;
    fwrite(&key, sizeof(int), 1, table->data_fp);
    fwrite(rec, table->rec_size, 1, table->data_fp);
    return PDS_SUCCESS;
}
...
int get_rec_by_key(char* table_name, int key, void* rec) {
    ...
    if (entry->is_deleted == REC_DELETED) {
        return PDS_REC_NOT_FOUND;
    }
    fseek(table->data_fp, entry->offset, SEEK_SET);
    int ckey;
    fread(&ckey, sizeof(int), 1, table->data_fp);
...
int get_rec_by_field(char* table_name, void* searchvalue, void* rec, int (*matcher)(void* rec, void* searchvalue), int* comp_count) {
    ...
    int search_count = 0;
    int ckey;
    while (fread(&ckey, sizeof(int), 1, table->data_fp) > 0) {
        search_count++;
        fread(rec, table->rec_size, 1, table->data_fp);
        if (matcher(rec, searchvalue) == 0) {
            struct BST_Node* node = bst_search(table->ndx_root, ckey);
            struct PDS_NdxInfo* entry = (struct PDS_NdxInfo*)node->data;
            if (entry->is_deleted == REC_DELETED) {
                continue;
            }
            else {
                *comp_count = search_count;
                return PDS_SUCCESS;
            }
        }
    }
...
int delete_rec_by_key(char* table_name, int key) {
    ...
    struct BST_Node* node = bst_search(table->ndx_root, key);
    struct PDS_NdxInfo* entry = (struct PDS_NdxInfo*)node->data;
    if (entry->is_deleted == REC_NOT_DELETED) {
        entry->is_deleted = REC_DELETED;
    }
    else {
        return PDS_REC_NOT_FOUND;
    }
...
void preorder(struct BST_Node* root, FILE* fp) {
    // Pre-order traversal
    if (root == NULL)
        return;
    else {
        struct PDS_NdxInfo* t = (struct PDS_NdxInfo*)root->data;

        fwrite(t, sizeof(struct PDS_NdxInfo), 1, fp);
        preorder(root->left_child, fp);
        preorder(root->right_child, fp);
    }
}
//////////////////////////////////////////////////////////////////////////////
// pds_close
...
int pds_close_table(char* table_name) {
    ...
    fwrite(&table->rec_count, sizeof(int), 1, fp);
    preorder(table->ndx_root, fp);
    bst_destroy(table->ndx_root);
    fclose(fp);
