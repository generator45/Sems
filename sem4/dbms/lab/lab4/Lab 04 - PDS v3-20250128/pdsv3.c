...
int put_rec_by_key(int key, void* rec) {
    ...
    // printf("recname in put rec by key %s\n", contact->contact_name);
    ...
    size_t recc = fwrite(rec, repo_handle.rec_size, 1, fp);
    if (recc < 1) {
        return PDS_ADD_FAILED;
    }
    return PDS_SUCCESS;
    ...
    if (repo_handle.repo_status == PDS_REPO_OPEN) {
        ...
        for (int i = 0; i < repo_handle.rec_count; i++) {
            if (repo_handle.ndx_array[i].key == key) {
                ...
                // printf("filekey in getfunc%d\n", filekey);
                if (filekey == key) {
                    fread(rec, repo_handle.rec_size, 1, fp);
                    struct Contact* contact = (struct Contact*)rec;
                    // printf("recname in getfunc%s\n", contact->contact_name);
                    return PDS_SUCCESS;
                }
                else {
...
// PDS_REC_NOT_FOUND - if the key is not found
// PDS_SUCCESS - function completes without any error
int get_rec_by_field(void* searchvalue, void* rec, int (*matcher)(void*, void*), int* comp_count) {
    if (repo_handle.repo_status != PDS_REPO_OPEN) {
        return PDS_REPO_NOT_OPEN;
    }
