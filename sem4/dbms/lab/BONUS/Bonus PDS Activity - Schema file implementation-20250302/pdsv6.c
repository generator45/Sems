// IMT2023111
// change create db >> same
// change open db ==> iterate thorugh the name after that iterate throught the name of the table then in thellp itself dcall populate the table info array
// in craeae table add an entry to the array and then call open table
// everything stays open
// close table not relevant
// create table takes rec size now
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
int pds_create_db(char* dbname) {
    ...
    char* dbinfo[40];
    sprintf(dbinfo, "%s.info", repo_handle.pds_repo_name);
    // [EDIT]
    FILE* fp = fopen(dbinfo, "wb");
    fseek(fp, 0, SEEK_CUR);
    for (int i = 0; i < repo_handle.num_tables; i++) {
        int rec_size;
        char* table_name[45];
        fread(table_name, sizeof(char), 45, fp);
...
int pds_create_table(char* table_name, int rec_size) {
    ...
    if (fp != NULL) {
        fseek(fp, 0, SEEK_END);
        fwrite(table_name, sizeof(*table_name), 1, fp);
